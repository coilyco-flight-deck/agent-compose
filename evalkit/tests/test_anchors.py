...
The roster is read from this checkout through `go run ./cmd/agent-compose
catalog roles --json` rather than an installed binary, so the check follows the
roster being edited. It came back from housecast under housecast#7961.
...
ANCHORS = ROOT / "evaluations" / "personality-anchors.yaml"
