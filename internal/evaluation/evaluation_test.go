...
func TestBuildUsesDesignerVisualImplementationBoundaryCases(t *testing.T) {
	...
	rolePrompt := pack.Cases[0].Prompt
	for _, want := range []string{
		"graphical React dashboard",
		"same props and state",
		"click handler and new interaction state",
		"terminal launch screen",
		"factory simulation",
		"procedural visual result generated by its game logic",
		"implement and deliver it, isolate a visual-only slice, or hand implementation",
		"product-effect boundary",
		"local verification and delivery follow-through",
	} {
		if !strings.Contains(rolePrompt, want) {
			t.Errorf("Designer role prompt omitted %q: %q", want, rolePrompt)
		}
	}
	if pack.Cases[2].Prompt != rolePrompt {
