...
func markdownScorecard(resultsDir, seat string, historical bool) ([]byte, error) {
	...
	for _, entry := range entries {
		...
		for _, scored := range result.Cases {
			...
			if evalCase.ScenarioKind == ScenarioAdjacentRole {
				cellKey = adjacentDimension
			}
			cell := model.cells[cellKey]
			if cell == nil {
				cell = &scorecardCell{passed: true}
				model.cells[cellKey] = cell
			}
	...
	if uniformModels && hasAdjacent {
		out.WriteString("| role | FR | FP | FA | OR | OP | OA | Σ |\n")
		out.WriteString("|---|---:|---:|---:|---:|---:|---:|---:|\n")
	} else if uniformModels {
		out.WriteString("| role | FR | FP | OR | OP | Σ |\n")
		out.WriteString("|---|---:|---:|---:|---:|---:|\n")
	} else if hasAdjacent {
		out.WriteString("| role | F model | O model | FR | FP | FA | OR | OP | OA | Σ |\n")
		out.WriteString("|---|---|---|---:|---:|---:|---:|---:|---:|---:|\n")
	} else {
...
func scorecardRowTotal(row scorecardRow) (int, int) {
	total := 0
	maximum := 0
	for _, model := range []*scorecardModel{row.frontier, row.oss} {
