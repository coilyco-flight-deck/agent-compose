...
func TestRoleScopedAppendixSkipsTheRoleLessLoadPoint(t *testing.T) {
	...
	base, err := OperatingBase(cfg, "claude", "eng-platform")
	if err != nil {
		t.Fatalf("operating base: %v", err)
	}
	if !strings.Contains(base, "Platform block.") || !strings.Contains(base, "Global block.") {
		t.Fatalf("eng-platform base missing a bound block:\n%s", base)
	}
	if !strings.Contains(base, "<!-- appendix: agent-compose.yaml entry 2 (roles: eng-platform, sysadmin) -->") {
...
func TestLoadConfigRejectsMalformedAppendixEntries(t *testing.T) {
	...
	for name, body := range map[string]string{
		...
		"empty role":         "appendix:\n  - text: hi\n    roles: [\"\"]\n",
		"repeated role":      "appendix:\n  - text: hi\n    roles: [eng-platform, eng-platform]\n",
...
func TestVerboseLayoutNamesEveryAppendixDestination(t *testing.T) {
	...
	for _, want := range []string{
		...
		"layout  <!-- appendix: agent-compose.yaml entry 2 (roles: eng-platform) --> => role bundles: eng-platform",
	} {
