...
func TestRenderSubagentsDecoratesEveryRowFromItsOwnDirectory(t *testing.T) {
	engineer := t.TempDir()
	writeProjectedRole(t, engineer, "claude", "eng-platform", "Angie")
	qa := t.TempDir()
	...
	if want := "🧭 ⛏️ Angie // eng-platform@claude"; rows[0].Content != want {
...
func TestRenderSubagentsSkipsRowsWithoutAProjection(t *testing.T) {
	projected := t.TempDir()
	writeProjectedRole(t, projected, "claude", "eng-platform", "Angie")
...
func TestRenderSubagentsPaintsOnlyWhenColorIsRequested(t *testing.T) {
	target := t.TempDir()
	writeProjectedRole(t, target, "claude", "eng-platform", "Angie")
	request, err := json.Marshal(SubagentRequest{Tasks: []SubagentTask{{ID: "task-1", CWD: target}}})
	if err != nil {
		t.Fatal(err)
	}
