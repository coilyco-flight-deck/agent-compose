...
* A Claude launch passes [identity flags](claude-launch-identity.md), not files, denies bare kubectl and helm to every role but the live-backend owner, and exports seat-labelled token metrics. Every native harness narrows MCP servers to the role. A failed [startup step](launch.md) degrades a launch if it shapes behavior and refuses it if it bounds reach.
