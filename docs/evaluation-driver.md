...
`--home` gives it an isolated authenticated home, separate from the driver's,
so it cannot inherit the role context projected for the driver. `--runs` takes
one file per arm and scores every arm in one blind pass. A case the driver
could not complete is skipped rather than scored.

It returns only a score and one evidence sentence per criterion, retries a
reply whose criteria, score range, or evidence do not hold, and exits non-zero
when a case ends unreviewed. `scripts/evaluation-record` derives totals and
verdicts from the pack's review rule, so they cannot drift with wording.
